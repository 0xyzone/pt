<x-base title="ENDING SOON">
    ...
    <style>
        ...
        /* Cyberpunk diagonal scanning line (Crimson theme) */
        .laser-sweep {
            position: absolute;
            inset: 0;
            background: linear-gradient(to bottom, transparent 40%, rgba(244, 63, 94, 0.04) 50%, transparent 60%);
        ...
        /* Pulsing neon lighting (Crimson theme) */
        .neon-glow-crimson {
            box-shadow: 0 0 35px rgba(244, 63, 94, 0.15), inset 0 0 25px rgba(244, 63, 94, 0.05);
        ...
        /* Animated radial lighting (Crimson exit theme) */
        .cyber-bg {
            background-color: #030712;
            background-image: 
                radial-gradient(at 15% 85%, rgba(244, 63, 94, 0.06) 0px, transparent 60%),
                radial-gradient(at 85% 15%, rgba(225, 29, 72, 0.06) 0px, transparent 60%);
        }
    ...
    <div id="screen-wrapper" class="w-full h-full min-h-screen {{ $bgType === 'animated' ? 'cyber-bg' : 'bg-transparent' }} relative flex flex-col justify-between items-center py-16 px-16 overflow-hidden text-white font-esports select-none">
        ...
        <div class="w-full max-w-6xl flex justify-between items-center z-10">
            ...
            <div class="hidden md:flex flex-col gap-1 text-left">
                ...
                <div class="w-24 h-1 bg-rose-500/35 rounded-full overflow-hidden">
                    <div class="w-2/3 h-full bg-rose-500 animate-pulse"></div>
                </div>
            ...
            <div class="flex items-center gap-6 justify-center mx-auto md:mx-0">
                <div class="bg-slate-950/90 border-2 border-rose-500/80 p-3.5 rounded-2xl shadow-xl drop-shadow-[0_0_15px_rgba(244,63,94,0.2)]">
                ...
                <div class="flex flex-col items-start text-left">
                    ...
                    <span class="text-3xl font-black uppercase text-rose-500 tracking-wider mt-1 drop-shadow-md">{{ $tournament->name }}</span>
            ...
            <div class="bg-rose-500/10 border border-rose-500/30 px-6 py-2.5 rounded-lg pubg-skew hidden md:block">
                <span class="pubg-unskew block text-rose-400 font-black uppercase tracking-[0.25em] text-sm animate-pulse">LIVE STREAM</span>
            </div>
        </div>
        ...
        <div class="flex flex-col items-center justify-center my-auto z-10 w-full max-w-4xl relative mt-8">
            ...
            <h1 class="text-7xl md:text-8xl font-black italic tracking-tighter uppercase mb-8 drop-shadow-2xl text-center leading-none">
                THANKS FOR <span class="text-rose-500 drop-shadow-[0_0_15px_rgba(244,63,94,0.35)]">WATCHING</span>
            </h1>

            {{-- Giant Timer HUD Container --}}
            <div id="timer-container" class="neon-glow-crimson bracket-corner bracket-inner w-full max-w-3xl bg-slate-950/80 rounded-2xl py-12 px-16 flex flex-col items-center justify-center shadow-2xl relative overflow-hidden backdrop-blur-md {{ !$timerState['visible'] ? 'hidden' : '' }}">
                ...
                <span class="text-lg uppercase font-bold text-slate-400 tracking-[0.35em] mt-2 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4 text-rose-500 animate-spin-slow">
                        <path fill-rule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zM12.75 6a.75.75 0 00-1.5 0v6c0 .414.336.75.75.75h4.5a.75.75 0 000-1.5h-3.75V6z" clip-rule="evenodd" />
                    </svg>
                    Tune back in shortly
                </span>
            </div>
        </div>
        ...
        <div class="w-full max-w-5xl z-10 flex flex-col items-center mt-auto">
            ...
            <div class="w-full h-24 glass-panel rounded-2xl flex items-center justify-center overflow-hidden relative shadow-lg">
                ...
                <div class="absolute left-0 inset-y-0 w-1.5 bg-rose-500"></div>
                <div class="absolute right-0 inset-y-0 w-1.5 bg-rose-500"></div>
                    ...
                    @foreach($sponsors as $idx => $sponsor)
                        <div class="sponsor-slide absolute inset-0 flex items-center justify-center p-4 transition-all duration-1000 opacity-0 transform translate-y-0 {{ $idx === 0 ? 'active opacity-100 translate-y-0' : '' }}" data-index="{{ $idx }}">
                            <div class="flex items-center gap-6">
                                ...
                                {{-- <span class="text-white font-black tracking-widest uppercase text-xl border-l-2 border-rose-500 pl-5 leading-none">{{ $sponsor->name }}</span> --}}
                            </div>
                        </div>
                    @endforeach
                @endif
