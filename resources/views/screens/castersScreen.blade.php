<x-base>
    ...
    <style>
        ...
        .bracket-corner::before { top: -2px; left: -2px; border-width: 3px 0 0 3px; }
        .bracket-corner::after { top: -2px; right: -2px; border-width: 3px 3px 0 0; }
        .bracket-inner::before { bottom: -2px; left: -2px; border-width: 0 0 3px 3px; }
        .bracket-inner::after { bottom: -2px; right: -2px; border-width: 0 3px 3px 0; }

        /* Gold highlight bracket for active styling */
        .bracket-gold::before, .bracket-gold::after, .bracket-gold-inner::before, .bracket-gold-inner::after {
            border-color: #eab308;
        }
        ...
        @keyframes holoFlicker {
            ...
            20%, 21.999%, 63%, 63.999%, 65%, 69.999% { opacity: 0.4; filter: hue-rotate(90deg); }
        }
    ...
    <div id="screen-wrapper" class="w-full h-full min-h-screen {{ $bgType === 'animated' ? 'cyber-bg' : 'bg-transparent' }} relative flex flex-col justify-between items-center py-10 px-12 overflow-hidden text-white font-esports select-none virtual-set-overlay">
        ...
        <div class="w-full max-w-[1720px] flex justify-between items-center z-20">
            {{-- Telemetry Info (Left) --}}
            ...
            @if($casters->count() != 2)
                <div class="flex items-center gap-4 bg-slate-900/60 border border-cyan-400/25 px-6 py-2.5 rounded-xl backdrop-blur-md shadow-lg">
                    <img src="{{ $tournament && $tournament->logo_image ? asset('storage/' . $tournament->logo_image) : asset('img/defult_team_logo.png') }}" 
                         class="w-10 h-10 object-contain" alt="Tournament Logo">
        ...
        <div class="w-full max-w-[1720px] flex justify-center items-center my-auto z-20">

            {{-- CASE 1: 1 CASTER --}}
            @if($casters->count() == 1)
                @php $caster = $casters->first(); @endphp
                <div class="flex flex-col items-center justify-center gap-6">
                    <div class="relative w-[860px] h-[483.75px] bg-slate-950/80 border border-cyan-400/25 rounded-lg overflow-hidden neon-glow-cyan bracket-corner bracket-inner flex items-center justify-center">
                        ...
                        @if ($caster->vdoninja_link)
                            <iframe src="{{ formatVdoNinjaUrl($caster->vdoninja_link) }}" 
                        ...
                        {{-- Nameplate overlay --}}
                        <div class="absolute bottom-0 inset-x-0 h-20 bg-gradient-to-t from-slate-950 via-slate-950/90 to-transparent z-20 flex flex-col justify-end px-8 pb-5">
            ...
            @elseif($casters->count() == 2 || $casters->count() == 0)
                ...
                <div class="flex items-center justify-between w-full gap-8">
                    ...
                    <div class="relative w-[620px] h-[348.75px] bg-slate-950/80 border border-cyan-400/25 rounded-lg overflow-hidden neon-glow-cyan bracket-corner bracket-inner flex items-center justify-center">
                        @if ($caster1)
                            <div class="absolute inset-0 w-full h-full z-0 overflow-hidden">
                                @if ($caster1->image)
                            ...
                            @if ($caster1->vdoninja_link)
                                <iframe src="{{ formatVdoNinjaUrl($caster1->vdoninja_link) }}" 
                                        class="absolute inset-0 w-full h-full border-0 z-10" 
                                        allow="autoplay;camera;microphone;fullscreen;picture-in-picture;display-capture" 
                                        allowtransparency="true">
                                </iframe>
                            ...
                            <div class="absolute bottom-0 inset-x-0 h-16 bg-gradient-to-t from-slate-950 via-slate-950/90 to-transparent z-20 flex flex-col justify-end px-5 pb-3">
                                <div class="flex justify-between items-end">
                                    <div class="flex flex-col text-left">
                    ...
                    <div class="flex flex-col items-center shrink-0 w-[400px]">
                        {{-- Futuristic Bezel / Outer shell --}}
                        <div class="w-full h-[255px] bg-slate-900 border-2 border-slate-800 rounded-2xl p-4 shadow-2xl relative overflow-hidden neon-glow-cyan">
                            ...
                            <div class="w-full h-full flex flex-col justify-between items-center relative z-10 py-1 text-center select-none font-esports">
                                ...
                                <img src="{{ $tournament && $tournament->logo_image ? asset('storage/' . $tournament->logo_image) : asset('img/defult_team_logo.png') }}" 
                                     class="w-16 h-16 object-contain filter drop-shadow-[0_0_10px_rgba(0,240,255,0.4)] my-1 hover:scale-105 transition-transform" alt="Tournament Logo">

                                {{-- Tournament Name --}}
                                <div class="flex flex-col">
                                    <span class="text-lg font-black uppercase tracking-wider text-slate-100 leading-none truncate max-w-[340px]">
                    ...
                    <div class="relative w-[620px] h-[348.75px] bg-slate-950/80 border border-cyan-400/25 rounded-lg overflow-hidden neon-glow-cyan bracket-corner bracket-inner flex items-center justify-center">
                        @if ($caster2)
                            ...
                            @if ($caster2->vdoninja_link)
                                <iframe src="{{ formatVdoNinjaUrl($caster2->vdoninja_link) }}" 
                                        class="absolute inset-0 w-full h-full border-0 z-10" 
                                        allow="autoplay;camera;microphone;fullscreen;picture-in-picture;display-capture" 
                                        allowtransparency="true">
                                </iframe>
            ...
            @elseif($casters->count() == 3)
                <div class="grid grid-cols-3 gap-6 w-full max-w-[1720px]">
                    @foreach($casters as $caster)
                        <div class="relative w-full h-[292.5px] bg-slate-950/80 border border-cyan-400/25 rounded-lg overflow-hidden neon-glow-cyan bracket-corner bracket-inner flex items-center justify-center">
                            ...
                            <div class="absolute bottom-0 inset-x-0 h-14 bg-gradient-to-t from-slate-950 via-slate-950/90 to-transparent z-20 flex flex-col justify-end px-5 pb-3">
                                <div class="flex justify-between items-end">
                                    <div class="flex flex-col text-left">
            ...
            @else
                <div class="grid grid-cols-2 gap-6 w-full max-w-[1240px]">
        ...
        <div class="w-full max-w-[1720px] flex justify-between items-center z-20 border-t border-slate-800/40 pt-4 telemetry-text font-orbitron">
